...
public class ShellViewModelTests
{
    ...
    public async Task SaveCommand_WithoutPath_PicksLocationThenSaves()
    {
        ...
        fileDialogs.PickSaveAsync(Arg.Any<string?>()).Returns("/tmp/diagram.draw");

        await shell.SaveCommand.ExecuteAsync(null);

        await files.Received(1).SaveAsync(Arg.Any<DiagramDocument>(), "/tmp/diagram.draw", Arg.Any<CancellationToken>());
        Assert.Equal("/tmp/diagram.draw", shell.ActiveDocument!.FilePath);
