...
public sealed class BezierRouter : IConnectorRouteStrategy
{
    ...
    public ConnectorRoute Route(ConnectorRouteRequest request)
    {
        ...
        Point2D outwardSource = RouteHelpers.SafeOutward(source - sourceCenter, target - source);
        Point2D outwardTarget = RouteHelpers.SafeOutward(target - targetCenter, source - target);
