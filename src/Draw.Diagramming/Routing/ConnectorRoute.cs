...
/// <summary>
/// The computed geometry of a connector: either a polyline (<see cref="Points"/>) or a smooth
/// multi-segment curve (<see cref="Cubics"/>). Endpoint direction unit vectors are provided for
/// orienting arrow/diamond/triangle decorations and are guaranteed non-zero (a default is
/// substituted for degenerate routes).
/// </summary>
public sealed class ConnectorRoute
{
    ...
    /// <summary>Ordered points. For a curve this is the polyline through the segment endpoints.</summary>
    public IReadOnlyList<Point2D> Points { get; }

    /// <summary>
    /// The ordered cubic segments of a smooth multi-segment curve, or null for polyline routes.
    /// When non-null the figure is <see cref="Start"/> followed by each segment's controls/end.
    /// </summary>
    ...
    public static ConnectorRoute Polyline(IReadOnlyList<Point2D> points)
    {
        ...
        return new ConnectorRoute(points, startDir, endDir);
    }
    ...
    public static ConnectorRoute PolyCubic(Point2D start, IReadOnlyList<CubicSegment> segments)
    {
        ...
        return new ConnectorRoute(points, startDir, endDir, segments);
    }
